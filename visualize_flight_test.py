...
def main():
    parser = argparse.ArgumentParser(
        description="Animate PATS insect tracker log in 3-D with pure pursuit overlay."
    )
    parser.add_argument(
        "csv",
        nargs="?",
        default="log_itrk3.csv",
        help="Path to the semicolon-delimited tracking CSV",
    )
    parser.add_argument(
        "--save",
        metavar="OUTPUT.mp4",
        default=None,
        help="Save animation to file instead of showing interactively",
    )
    parser.add_argument(
        "--fps",
        type=int,
        default=FPS,
        help=f"Frames per second for playback / export (default: {FPS})",
    )
    parser.add_argument(
        "--speed",
        type=float,
        default=PURSUER_SPEED,
        help=f"Pursuer speed in m/s (default: {PURSUER_SPEED})",
    )
    parser.add_argument(
        "--turn-rate",
        type=float,
        default=PURSUER_MAX_TURN,
        help=f"Pursuer max turn rate in rad/s (default: {PURSUER_MAX_TURN}, 0 = perfect)",
    )
    ...
    print(f"  {len(df)} valid frames  |  "
          f"duration {df['elapsed'].iloc[-1] - df['elapsed'].iloc[0]:.2f} s")

    fig, anim = build_animation(df)

    if args.save:
        print(f"Saving to {args.save} …")
        writer = animation.FFMpegWriter(fps=args.fps, bitrate=1800)
        anim.save(args.save, writer=writer, dpi=150,
