...
class QMILInterface:
    def __init__(self, name="Propeller"):
        ...
        self.op_point = {"vel": 5.0, "rpm": 40000.0}
        self.targets = {"thrust": 0.0, "power": 0}  # thrust set dynamically via T/W
        self.ldes = 0 
        self.kqdes = 0
        self.n_out = 30
...
def run_and_parse(qmil_obj, qprop_obj, vel="5.0", rpm_sweep="20000,50000/8"):
...
s_rpmdes  = Slider(plt.axes([0.15, 0.16, 0.25, 0.025], facecolor=ax_color), 'Des RPM', 5000, 50000, valinit=40000, valstep=500)
s_hub     = Slider(plt.axes([0.15, 0.12, 0.25, 0.025], facecolor=ax_color), 'Hub Rad [m]', 0.0001, 0.02, valinit=0.00075)
s_tip     = Slider(plt.axes([0.15, 0.08, 0.25, 0.025], facecolor=ax_color), 'Tip Rad [m]', 0.01, 0.2, valinit=0.020)
...
s_io = Slider(plt.axes([0.65, 0.16, 0.25, 0.025], facecolor=ax_color), 'Motor $I_0$ [A]', 0.1, 5.0, valinit=1.5)
s_kv = Slider(plt.axes([0.65, 0.12, 0.25, 0.025], facecolor=ax_color), 'Motor $K_v$ [rpm/V]', 500, 30000, valinit=15000, valstep=50)
s_tw = Slider(plt.axes([0.65, 0.08, 0.25, 0.025], facecolor=ax_color), 'Thrust/Weight', 0.5, 10.0, valinit=2.0, valstep=0.1)
