...
class AeroEngine:
    ...
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, cd_csv_path: str, body_name: str="drone", rho: float=1.225, area: float=0.004) -> None:
        ...
        #decimal=',' handles European number format (e.g. "1,248" instead of "1.248")
        df=pd.read_csv(cd_csv_path, decimal=',', usecols=[0, 1])
        #ASSUME FOR NOW THAT COLUMN 1 IS ANGLES AND COLUMN 2 is CD
        self.angles_ref = df.iloc[:,0].to_numpy(dtype=float)
        self.cds_ref = df.iloc[:,1].to_numpy(dtype=float)
    ...
    def body_velocity(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        return self._vel6_world[3:6], self._vel6_local[3:6]
